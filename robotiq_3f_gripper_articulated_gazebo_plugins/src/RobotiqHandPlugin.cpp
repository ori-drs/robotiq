...
void RobotiqHandPlugin::UpdatePIDControl(double _dt)
{
    ...
    for (int i = 0; i < this->NumJoints; ++i)
    {
...
#if GAZEBO_MAJOR_VERSION >= 9
                    targetPose = this->joints[i]->UpperLimit(0) -
                                 (this->joints[i]->UpperLimit(0) - this->joints[i]->LowerLimit(0)) * (215.0 / 255.0)
#else
                    targetPose =
        ...
        else if (i >= 2 && i <= 4)
        {
...
#if GAZEBO_MAJOR_VERSION >= 9
                targetPose = this->joints[i]->LowerLimit(0) +
                             (this->joints[i]->UpperLimit(0) - this->joints[i]->LowerLimit(0)) * (177.0 / 255.0)
#else
                targetPose = this->joints[i]->GetLowerLimit(0).Radian() +
                             (this->joints[i]->GetUpperLimit(0).Radian() - this->joints[i]->GetLowerLimit(0).Radian()) *
                                 (177.0 / 255.0)
            ...
            {
                targetSpeed =
                    this->MinVelocity + ((this->MaxVelocity - this->MinVelocity) * this->handleCommand.rSPA / 255.0);
            }
            else
            {
#if GAZEBO_MAJOR_VERSION >= 9
                targetPose =
                    this->joints[i]->LowerLimit(0) + (this->joints[i]->UpperLimit(0) - this->joints[i]->LowerLimit(0))
#else
                targetPose = this->joints[i]->GetLowerLimit(0).Radian() +
                             (this->joints[i]->GetUpperLimit(0).Radian() - this->joints[i]->GetLowerLimit(0).Radian())
#endif
