...
#include <robotiq_2f_gripper_articulated_gazebo_plugins/RobotiqHandPlugin.h>
...
void RobotiqHandPlugin::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    ...
    // ROS topic names.
    this->topicCommand = prefix + std::string("/command");
    this->topicState = prefix + std::string("/state");
    ...
    this->pubJointStates =
        this->rosNode->advertise<sensor_msgs::JointState>(this->prefix + std::string("/joint_states"), 10);
    ...
    const robotiq_2f_gripper_control::Robotiq2FGripper_robot_output::ConstPtr& _command)
{
    ...
    //        this->VerifyField("rPRA", 0, 255, _command->rACT) && this->VerifyField("rSPA", 0, 255, _command->rACT) &&
    //        this->VerifyField("rFRA", 0, 255, _command->rACT) && this->VerifyField("rPRB", 0, 255, _command->rACT) &&
    //        this->VerifyField("rSPB", 0, 255, _command->rACT) && this->VerifyField("rFRB", 0, 255, _command->rACT) &&
    //        this->VerifyField("rPRC", 0, 255, _command->rACT) && this->VerifyField("rSPC", 0, 255, _command->rACT) &&
    //        this->VerifyField("rFRC", 0, 255, _command->rACT) && this->VerifyField("rPRS", 0, 255, _command->rACT) &&
    //        this->VerifyField("rSPS", 0, 255, _command->rACT) && this->VerifyField("rFRS", 0, 255, _command->rACT);
    return true;
}
...
bool RobotiqHandPlugin::IsHandFullyOpen()
{
    ...
    // The hand will be fully open when all the joints are within 'tolerance'
    ...
    // The first joint (0) is the actuated joint. The subsequent joints are mimic joints
...
void RobotiqHandPlugin::GetAndPublishHandleState()
{
    ...
    this->handleState.gDTA =
        this->GetObjectDetection(this->joints[2], 2, this->handleCommand.rPRA, this->prevCommand.rPRA);

    // gDTB. Finger B object detection.
    this->handleState.gDTB =
        this->GetObjectDetection(this->joints[3], 3, this->handleCommand.rPRB, this->prevCommand.rPRB);

    // gDTC. Finger C object detection
    this->handleState.gDTC =
        this->GetObjectDetection(this->joints[4], 4, this->handleCommand.rPRC, this->prevCommand.rPRC);

    // gDTS. Scissor object detection. We use finger A as a reference.
    this->handleState.gDTS =
        this->GetObjectDetection(this->joints[0], 0, this->handleCommand.rPRS, this->prevCommand.rPRS);
...
void RobotiqHandPlugin::UpdatePIDControl(double _dt)
{
    ...
    for (int i = 0; i < this->NumJoints; ++i)
    {
        ...
        targetSpeed =
            this->MinVelocity + ((this->MaxVelocity - this->MinVelocity) * this->handleCommand.rSP / 255.0);

        // TODO: Flip based on ID ... qqq
...
////////////////////////////////////////////////////////////////////////////////
bool RobotiqHandPlugin::GetAndPushBackJoint(const std::string& _jointName, gazebo::physics::Joint_V& _joints)
{
...
bool RobotiqHandPlugin::FindJoints()
{
    ...
    // right_outer_knuckle_joint (mimic, -1).
    suffix = "right_outer_knuckle_joint";
    if (!this->GetAndPushBackJoint(prefix + suffix, this->joints))
        return false;
    // if (!this->GetAndPushBackJoint(prefix + suffix, this->fingerJoints))
    //     return false;
    ...
    /*qqq // We read the joint state from finger_1_joint_1
